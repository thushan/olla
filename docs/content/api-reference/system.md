...
- **Weak ETag.** Every `200` response carries an `ETag` header of the form `W/"<base36>"`, a 64-bit FNV-1a hash rendered in base36. It is a **weak** validator per RFC 7232: the hash covers the operator-visible state but deliberately excludes every per-second relative string (`uptime`, `last_check`, `next_check`, `last_seen`, `last_model_sync`) and the top-level `timestamp`. Two payloads a second apart under unchanged state hash identically, but the hash is not a byte-for-byte content identity.
...
- **`Cache-Control: private, no-cache`.** Both the `200` and `304` responses carry this header. `private` marks the payload unsuitable for any shared cache; `no-cache` (not `no-store`) permits the client to store the body but requires revalidation before reuse, which is exactly the `If-None-Match` round trip above - a client stores the last `200`, then revalidates on the next poll and gets a bare `304` when nothing changed.
