...
*Allowlist note (build-pipeline fix, added after the original spec draft):* `.github/workflows/release.yml` was missing from the allowlist below even though `.github/workflows/ci.yml` was already present. That was an oversight, not a deliberate exclusion — this branch's `build: build-web` addition (`be7b0cb`) made `.goreleaser.yml`'s `before.hooks` depend on Node for the first time, and `release.yml` is the workflow that runs goreleaser for tagged releases, so it is exactly as in-scope as `ci.yml` was for the equivalent CI fix. Added `\.github/workflows/release\.yml$` to the pattern.

```
git diff --name-only main...HEAD | grep -vE '^(web/dashboard/|internal/app/handlers/dashboard/|internal/app/handlers/handler_status\.go$|internal/app/handlers/handler_status_endpoints\.go$|internal/app/handlers/handler_status_models\.go$|internal/app/handlers/handler_status.*_test\.go$|internal/app/handlers/handler_routes_dashboard_test\.go$|internal/app/handlers/server_routes\.go$|internal/app/middleware/logging\.go$|internal/app/middleware/logging.*_test\.go$|internal/config/types\.go$|internal/config/config\.go$|internal/config/dashboard_test\.go$|internal/config/shipped_config_test\.go$|internal/adapter/discovery/repository\.go$|internal/adapter/discovery/repository.*_test\.go$|config/config\.yaml$|test/config_docs/|makefile$|\.github/workflows/ci\.yml$|\.github/workflows/release\.yml$|\.goreleaser\.yml$|\.gitignore$|docs/)'
```
