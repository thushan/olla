...
type ProxyTestSuite interface {
	Name() string
	CreateProxy(discovery ports.DiscoveryService, selector domain.EndpointSelector, config ports.ProxyConfiguration) ports.ProxyService
...
func (s SherpaTestSuite) CreateProxy(discovery ports.DiscoveryService, selector domain.EndpointSelector, config ports.ProxyConfiguration) ports.ProxyService {
	return NewSherpaService(discovery, selector, config.(*Configuration), createTestLogger())
}
...
func testProxyRequestSuccess(t *testing.T, suite ProxyTestSuite) {
	...
	proxy := suite.CreateProxy(discovery, selector, config)
